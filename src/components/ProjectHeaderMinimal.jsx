...
function ProjectHeaderMinimal({ projectData }) {
    const {
        title,
        imgSrc,
        demoVideo,
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-[400px]">
                ...
                <div className="bg-gradient-to-b from-boxColorA to-boxColorB rounded-lg p-8 flex flex-col justify-between">
                    <div>
                        ...
                        <div className="text-white/90 leading-relaxed prose-sm">
                            <div dangerouslySetInnerHTML={{ __html: description }} />
                        </div>
                    </div>
                    {/* Details as inline tags */}
                    <div className="flex flex-wrap gap-3 mt-6">
                        {details.map((detail, index) => {
                            const [key, value] = Object.entries(detail)[0]
                            return (
                                <span key={index} className="bg-black/30 px-3 py-1 rounded-full text-sm">
...
export default ProjectHeaderMinimal 